...
import platform
import os
import urllib.request # Import untuk request ke internet
...
# --- WORKER 1: SYSTEM MONITOR (CPU/RAM/NET SPEED) ---
...
# --- WORKER 2: PUBLIC IP CHECKER (Jalan Sekali Saja) ---
class PublicIpWorker(QThread):
    ip_signal = Signal(str)

    def run(self):
        try:
            # Request ke layanan cek IP gratis (timeout 5 detik biar ga lama loading)
            url = "https://api.ipify.org"
            with urllib.request.urlopen(url, timeout=5) as response:
                public_ip = response.read().decode('utf-8')
                self.ip_signal.emit(public_ip)
        except Exception:
            self.ip_signal.emit("Offline / N/A")
...
class WidgetMonitor(QWidget):
    def __init__(self):
        ...
        # 1. Start System Monitor (CPU/RAM)
        self.monitor_thread = SystemMonitor()
        self.monitor_thread.stats_signal.connect(self.update_stats)
        self.monitor_thread.daemon = True # Auto kill saat close
        self.monitor_thread.start()

        # 2. Start Public IP Checker
        self.ip_thread = PublicIpWorker()
        self.ip_thread.ip_signal.connect(self.update_ip)
    ...
    def update_ip(self, ip_address):
        self.lbl_ip_val.setText(ip_address)
